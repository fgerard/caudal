...
(ns mx.interware.caudal.test.simple-config
  (:require [clojure.pprint :as pp]
            [clojure.string :as str]
            [clojure.tools.logging :as log]
            [clojure.walk :refer [walk]]
            [clojure.core.async :as async :refer [chan go go-loop timeout <! >! <!! >!!]]
            [mx.interware.caudal.core.state :refer [as-map lookup]]
            [mx.interware.caudal.streams.common :refer [create-sink key-factory defstream
                                                        using-path using-path-with-default-streams]]
            [mx.interware.caudal.streams.stateful :refer [batch concurrent-meter counter
                                                          changed matcher reduce-with
                                                          acum-stats dump-every
                                                          ewma-timeless rate uneven-tx welford]]
            [mx.interware.caudal.streams.stateless :refer [->DEBUG ->INFO ->WARN ->ERROR
