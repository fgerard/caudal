...
(ns mx.interware.caudal.io.server
...
(defn fast-echo-handler
  [f]
  (fn [s info]
    (log/debug :fast-echo-handler :info info)
    (s/connect
      (s/map f s)
      s)))
...
(comment def s
         (start-server
           (fast-echo-handler (fn [p] (log/debug :p p) "ok"))
           10000))
...
(comment do
         (log/debug @(d/timeout! (s/take! c) 10000 :timeout))
         (log/debug @(d/timeout! (s/take! c) 10000 :timeout))
         (log/debug @(d/timeout! (s/take! c) 10000 :timeout)))

;@(s/put! c {:uno 1 :paterno "gerard"})
